...
PORT="${1:-8387}"
URL="http://localhost:${PORT}/api/v0/calendars/gregorian/dates/today"
...
if ! grep -q '"date"' "$tmp_body" || ! grep -q '"equivalents"' "$tmp_body"; then
  echo "FAIL: endpoint JSON does not contain expected keys (date, equivalents)" >&2
  echo "---- response body ----" >&2
  cat "$tmp_body" >&2
  exit 1
fi
